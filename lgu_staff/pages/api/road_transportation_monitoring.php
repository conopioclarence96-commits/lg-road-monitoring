...
function getRecentSubmissions($limit = 10, $status_filter = 'all', $type_filter = 'all', $transport_only = false) {
    ...
    try {
        ...
        //    cimm_status_case_sql()) instead of a fixed 'completed' — see
        //    that function's docblock for why. The outer SELECT * wrapper
        //    turns the mapped value into a real column so the status_filter
        //    appended by $fetch() (AND status = ?) can still match on it.
        if (!$transport_only) {
            try {
            $reports = array_merge($reports, $fetch(
                "SELECT * FROM (
