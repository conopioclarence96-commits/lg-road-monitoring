...
switch ($action) {
    ...
    case 'create':
        ...
        if ($stmt->execute()) {
            ...
            log_audit_action($_SESSION['user_id'], 'create_completed_project', "Created project #$new_id: $title");
            echo json_encode(['success' => true, 'message' => 'Project created', 'id' => $new_id]);
        } else {
    ...
    case 'update':
        ...
        $stmt = $conn->prepare("UPDATE published_completed_projects SET title=?, description=?, location=?, completed_date=?, cost=?, completed_by=?, photo=?, before_photo=? WHERE id=?");
        $stmt->bind_param('ssssdsssi', $title, $description, $location, $date_val, $cost, $completed_by, $photo, $before_photo, $id);

        if ($stmt->execute()) {
            log_audit_action($_SESSION['user_id'], 'update_completed_project', "Updated project #$id: $title");
