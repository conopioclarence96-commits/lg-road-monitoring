...
function handle_update_report() {
    ...
    if (!empty($uploaded_photos) && $table === 'road_transportation_reports') {
        $existing = $conn->query("SELECT attachments, image_path FROM {$table} WHERE id = {$report_id}")->fetch_assoc();
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .table-action-edit:hover { background: #ffc107; color: #fff; }
        .table-action-updates {
            background: rgba(16,185,129,0.12); color: #059669;
        }
        .table-action-updates:hover { background: #10b981; color: #fff; }
        ...
        .form-section h6 i { color: #3762c8; font-size: 14px; }
        ...
        .form-section .form-control { font-size: 13px; padding: 8px 12px; }
        input[type="file"].form-control { padding: 6px 10px; font-size: 12px; }
        ...
        #existingPhotos img, #photoPreview img { transition: transform 0.2s; }
        #existingPhotos img:hover, #photoPreview img:hover { transform: scale(1.05); }
...
<body class="<?php echo !empty($_SESSION['darkmode']) ? 'dark-mode' : ''; ?>">
                        ...
                        <?php foreach ($recent_reports as $rr): ?>
                         <tr class="report-table-row" data-id="<?php echo $rr['id']; ?>" data-title="<?php echo htmlspecialchars(strtolower($rr['title'] ?? '')); ?>" data-report-id="<?php echo htmlspecialchars(strtolower($rr['report_id'] ?? '')); ?>" data-status="<?php echo $rr['status'] ?? 'pending'; ?>">
                            ...
                            <td style="white-space:nowrap;">
                                ...
                                <button class="table-action-btn table-action-edit" onclick="editReport(<?php echo $rr['id']; ?>, '<?php echo $rr['report_type']; ?>')"><i class="fas fa-pencil"></i> Edit</button>
                                <button class="table-action-btn table-action-updates" onclick="viewReportUpdates(<?php echo $rr['id']; ?>, '<?php echo $rr['report_type']; ?>')"><i class="fas fa-clock"></i> Updates</button>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                        <?php endif; ?>
    ...
    <script>
            ...
            fetch('../api/get_report_details.php?id=' + id + '&type=' + type)
                ...
                .then(function(data) {
                    if (data.success) {
                        ...
                        var container = document.getElementById('existingPhotos');
                        container.innerHTML = '';
                        var hasPhotos = false;
                        if (data.report.attachments) {
                            try {
                                var attachments = JSON.parse(data.report.attachments);
                                if (Array.isArray(attachments) && attachments.length) {
                                    attachments.forEach(function(a) {
                                        if (a.type === 'image' && a.file_path) {
                                            hasPhotos = true;
                                            var wrap = document.createElement('div');
                                            wrap.style.cssText = 'position:relative;width:100px;height:100px;border-radius:8px;overflow:hidden;border:1px solid rgba(55,98,200,0.2);flex-shrink:0;';
                        ...
                        document.getElementById('updateStatusIndicator').textContent = 'Last updated: ' + (data.report.updated_at || 'N/A');
        ...
        // Photo preview
        var editPhotosInput = document.getElementById('editPhotos');
        var photoPreview = document.getElementById('photoPreview');
        var addEditPhotosBtn = document.getElementById('add-edit-photos-btn');
        ...
        function renderEditGallery() {
            ...
            editSelectedFiles.forEach(function(file, idx) {
                var reader = new FileReader();
                reader.onload = function(e) {
                    var wrap = document.createElement('div');
                    wrap.style.cssText = 'position:relative;width:80px;height:80px;border-radius:6px;overflow:hidden;border:1px solid #ddd;flex-shrink:0;background:#f8f9fa;';
                    var img = document.createElement('img');
                    ...
                    del.type = 'button';
                    del.innerHTML = '&times;';
                    del.style.cssText = 'position:absolute;top:2px;right:2px;width:20px;height:20px;background:rgba(220,53,69,0.9);color:#fff;border:none;border-radius:50%;font-size:14px;line-height:1;cursor:pointer;display:flex;align-items:center;justify-content:center;';
                    del.onclick = function() { editSelectedFiles.splice(idx, 1); renderEditGallery(); };
                    wrap.appendChild(img);
        ...
        // Form submission
        var editForm = document.getElementById('editReportForm');
        if (editForm) {
            editForm.addEventListener('submit', function(e) {
                e.preventDefault();
                var dt = new DataTransfer();
                editSelectedFiles.forEach(function(f) { dt.items.add(f); });
                editPhotosInput.files = dt.files;
                var formData = new FormData(this);
                var submitBtn = this.querySelector('button[type="submit"]');
                var originalText = submitBtn.innerHTML;
                var indicator = document.getElementById('updateStatusIndicator');
