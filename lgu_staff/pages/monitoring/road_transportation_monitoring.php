        ...
        case 'submit_report':
            ...
            try {
                ...
                if (!empty($_FILES['report_images']) && is_array($_FILES['report_images']['name'])) {
                    $file_count = count($_FILES['report_images']['name']);
                    for ($i = 0; $i < $file_count; $i++) {
                        if ($_FILES['report_images']['error'][$i] === UPLOAD_ERR_NO_FILE) continue;
                        if ($_FILES['report_images']['error'][$i] !== UPLOAD_ERR_OK) {
                            ...
                            $error_code = $_FILES['report_images']['error'][$i];
                            $error_msg = $upload_errors[$error_code] ?? 'Unknown error (code: ' . $error_code . ')';
                            echo json_encode(['success' => false, 'message' => "Upload failed for '{$_FILES['report_images']['name'][$i]}': $error_msg"]);
                            exit;
                        }
                        
                        $file = [
                            'name' => $_FILES['report_images']['name'][$i],
                            'type' => $_FILES['report_images']['type'][$i],
                            'tmp_name' => $_FILES['report_images']['tmp_name'][$i],
                            'error' => $_FILES['report_images']['error'][$i],
                            'size' => $_FILES['report_images']['size'][$i]
                        ];
            ...
            <div class="map-section">
                ...
                <div id="report-form-panel" class="report-form-panel" style="display: none;">
                    ...
                    <form id="report-form">
                        ...
                        <input type="file" id="report-images" name="report_images[]" accept="image/*" multiple />
                        ...
                        <div id="image-previews" style="margin-top: 10px; display: none; display: flex; gap: 8px; flex-wrap: wrap;"></div>
