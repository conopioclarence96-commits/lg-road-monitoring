...
try {
    ...
    $query = "
        SELECT i.*, 
               CASE 
                   WHEN i.inspector_id = 999999 THEN 'Citizen Report'
                   ELSE COALESCE(u.name, 'Unknown')
               END as reporter_name,
               CASE 
                   WHEN i.inspector_id = 999999 THEN 'citizen'
                   ELSE 'regular'
               END as inspection_type
        FROM inspections i 
        LEFT JOIN users u ON i.inspector_id = u.id AND i.inspector_id != 999999
